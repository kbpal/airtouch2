...
public class Answers extends Kit<Boolean> {
    ...
    public boolean onPreExecute() {
        ...
        try {
            ...
            Fabric.getLogger().mo6e(TAG, "Error retrieving app properties", e);
            return false;
        }
    }
    ...
    public Boolean doInBackground() {
        if (!new FirebaseInfo().isDataCollectionDefaultEnabled(getContext())) {
            Fabric.getLogger().mo9d(Fabric.TAG, "Analytics collection disabled, because data collection is disabled by Firebase.");
        ...
        try {
            ...
            if (awaitSettingsData == null) {
                Fabric.getLogger().mo7e(TAG, "Failed to retrieve settings");
                return false;
            } else if (awaitSettingsData.featuresData.collectAnalytics) {
                Fabric.getLogger().mo9d(TAG, "Analytics collection enabled");
                this.analyticsManager.setAnalyticsSettingsData(awaitSettingsData.analyticsSettingsData, getOverridenSpiEndpoint());
                return true;
            } else {
                Fabric.getLogger().mo9d(TAG, "Analytics collection disabled");
                this.analyticsManager.disable();
                return false;
            }
        } catch (Exception e) {
            Fabric.getLogger().mo6e(TAG, "Error dealing with settings", e);
            return false;
        }
    }
    ...
    private void logFirebaseModeEnabledWarning(String str) {
        ...
        logger.mo1w(TAG, "Method " + str + " is not supported when using Crashlytics through Firebase.");
    }
}
