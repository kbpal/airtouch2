...
class EnabledSessionAnalyticsManagerStrategy implements SessionAnalyticsManagerStrategy {
    ...
    public void setAnalyticsSettingsData(AnalyticsSettingsData analyticsSettingsData, String str) {
        ...
        logger3.mo9d(Answers.TAG, sb3.toString());
        ...
        logger4.mo9d(Answers.TAG, sb4.toString());
        if (analyticsSettingsData.samplingRate > 1) {
            Fabric.getLogger().mo9d(Answers.TAG, "Event sampling enabled");
    ...
    public void processEvent(SessionEvent.Builder builder) {
        ...
        if (!this.customEventsEnabled && SessionEvent.Type.CUSTOM.equals(build.type)) {
            Logger logger = Fabric.getLogger();
            logger.mo9d(Answers.TAG, "Custom events tracking disabled - skipping event: " + build);
        } else if (!this.predefinedEventsEnabled && SessionEvent.Type.PREDEFINED.equals(build.type)) {
            Logger logger2 = Fabric.getLogger();
            logger2.mo9d(Answers.TAG, "Predefined events tracking disabled - skipping event: " + build);
        } else if (this.eventFilter.skipEvent(build)) {
            Logger logger3 = Fabric.getLogger();
            logger3.mo9d(Answers.TAG, "Skipping filtered event: " + build);
        } else {
            try {
                this.filesManager.writeEvent(build);
            } catch (IOException e) {
                Logger logger4 = Fabric.getLogger();
                logger4.mo6e(Answers.TAG, "Failed to write event: " + build, e);
            }
            ...
            if (this.forwardToFirebaseAnalyticsEnabled && z) {
                if (!equals || this.includePurchaseEventsInForwardedEvents) {
                    try {
                        ...
                        logger5.mo6e(Answers.TAG, "Failed to map event to Firebase: " + build, e2);
                    }
                }
            }
        }
