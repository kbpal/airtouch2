...
public class FirebaseAnalyticsApiAdapter {
    ...
    public void processEvent(SessionEvent sessionEvent) {
        ...
        if (firebaseAnalytics == null) {
            Fabric.getLogger().mo9d(Answers.TAG, "Firebase analytics logging was enabled, but not available...");
            return;
        }
        FirebaseAnalyticsEvent mapEvent = this.eventMapper.mapEvent(sessionEvent);
        if (mapEvent == null) {
            Logger logger = Fabric.getLogger();
            logger.mo9d(Answers.TAG, "Fabric event was not mappable to Firebase event: " + sessionEvent);
            return;
        }
