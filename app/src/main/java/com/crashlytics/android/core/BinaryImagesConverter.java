...
class BinaryImagesConverter {
    ...
    private JSONArray parseProcMapsJsonFromString(String str) {
        ...
        try {
            ...
            Fabric.getLogger().mo0w(CrashlyticsCore.TAG, "Unable to parse proc maps string", e);
            return jSONArray;
        }
    }
    ...
    private JSONObject jsonFromMapEntryString(String str) {
        ...
        try {
            try {
                ...
                Fabric.getLogger().mo8d(CrashlyticsCore.TAG, "Could not create a binary image json string", e);
                return null;
            }
        } catch (IOException e2) {
            Logger logger = Fabric.getLogger();
            logger.mo8d(CrashlyticsCore.TAG, "Could not generate ID for file " + parse.path, e2);
            return null;
        }
    }
    ...
    private File correctDataPath(File file) {
        if (Build.VERSION.SDK_INT >= 9 && file.getAbsolutePath().startsWith(DATA_DIR)) {
            try {
                ...
                Fabric.getLogger().mo6e(CrashlyticsCore.TAG, "Error getting ApplicationInfo", e);
                return file;
            }
        }
        return file;
    ...
    private static byte[] generateBinaryImagesJsonString(JSONArray jSONArray) {
        ...
        try {
            ...
            Fabric.getLogger().mo0w(CrashlyticsCore.TAG, "Binary images string is null", e);
            return new byte[0];
        }
    }
