...
class DefaultCreateReportSpiCall extends AbstractSpiCall implements CreateReportSpiCall {
    ...
    public boolean invoke(CreateReportRequest createReportRequest) {
        ...
        logger.mo9d(CrashlyticsCore.TAG, "Sending report to: " + getUrl());
        int code = applyMultipartDataTo.code();
        Logger logger2 = Fabric.getLogger();
        logger2.mo9d(CrashlyticsCore.TAG, "Create report request ID: " + applyMultipartDataTo.header(AbstractSpiCall.HEADER_REQUEST_ID));
        Logger logger3 = Fabric.getLogger();
        logger3.mo9d(CrashlyticsCore.TAG, "Result was: " + code);
    ...
    private HttpRequest applyMultipartDataTo(HttpRequest httpRequest, Report report) {
        ...
        if (report.getFiles().length == 1) {
            Fabric.getLogger().mo9d(CrashlyticsCore.TAG, "Adding single file " + report.getFileName() + " to report " + report.getIdentifier());
            return httpRequest.part(FILE_PARAM, report.getFileName(), FILE_CONTENT_TYPE, report.getFile());
        }
        int i = 0;
        for (File file : report.getFiles()) {
            Fabric.getLogger().mo9d(CrashlyticsCore.TAG, "Adding file " + file.getName() + " to report " + report.getIdentifier());
