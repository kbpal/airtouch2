...
public class Onboarding extends Kit<Boolean> {
    ...
    public boolean onPreExecute() {
        try {
            ...
            Fabric.getLogger().mo6e(Fabric.TAG, "Failed init", e);
            return false;
        }
    }
    ...
    public Boolean doInBackground() {
        ...
        if (retrieveSettingsData != null) {
            try {
                ...
                Fabric.getLogger().mo6e(Fabric.TAG, "Error performing auto configuration.", e);
            }
    ...
    private SettingsData retrieveSettingsData() {
        try {
            ...
            Fabric.getLogger().mo6e(Fabric.TAG, "Error dealing with settings", e);
            return null;
        }
    }
    ...
    private boolean performAutoConfigure(String str, AppSettingsData appSettingsData, Collection<KitInfo> collection) {
        if (AppSettingsData.STATUS_NEW.equals(appSettingsData.status)) {
            ...
            Fabric.getLogger().mo6e(Fabric.TAG, "Failed to create app with Crashlytics service.", null);
            return false;
        } else if (AppSettingsData.STATUS_CONFIGURED.equals(appSettingsData.status)) {
            return Settings.getInstance().loadSettingsSkippingCache();
        } else {
            if (appSettingsData.updateRequired) {
                Fabric.getLogger().mo9d(Fabric.TAG, "Server says an update is required - forcing a full App update.");
