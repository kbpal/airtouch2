...
class AdvertisingInfoProvider {
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        ...
        if (isInfoValid(infoFromPreferences)) {
            Fabric.getLogger().mo9d(Fabric.TAG, "Using AdvertisingInfo from Preference Store");
    ...
    private void refreshInfoIfNeededAsync(final AdvertisingInfo advertisingInfo) {
        new Thread(new BackgroundPriorityRunnable() { // from class: io.fabric.sdk.android.services.common.AdvertisingInfoProvider.1
            ...
            public void onRun() {
                ...
                Fabric.getLogger().mo9d(Fabric.TAG, "Asychronously getting Advertising Info and storing it to preferences");
    ...
    public AdvertisingInfo getAdvertisingInfoFromStrategies() {
        ...
        if (!isInfoValid(advertisingInfo)) {
            ...
            if (!isInfoValid(advertisingInfo)) {
                Fabric.getLogger().mo9d(Fabric.TAG, "AdvertisingInfo not present");
            } else {
                Fabric.getLogger().mo9d(Fabric.TAG, "Using AdvertisingInfo from Service Provider");
            }
        } else {
            Fabric.getLogger().mo9d(Fabric.TAG, "Using AdvertisingInfo from Reflection Provider");
        }
