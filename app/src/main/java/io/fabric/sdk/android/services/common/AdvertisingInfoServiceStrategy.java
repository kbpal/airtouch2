...
public class AdvertisingInfoServiceStrategy implements AdvertisingInfoStrategy {
    ...
    public AdvertisingInfo getAdvertisingInfo() {
        if (Looper.myLooper() == Looper.getMainLooper()) {
            Fabric.getLogger().mo9d(Fabric.TAG, "AdvertisingInfoServiceStrategy cannot be called on the main thread");
            return null;
        }
        try {
            ...
            try {
                if (this.context.bindService(intent, advertisingConnection, 1)) {
                    try {
                        ...
                        Fabric.getLogger().mo0w(Fabric.TAG, "Exception in binding to Google Play Service to capture AdvertisingId", e);
                        this.context.unbindService(advertisingConnection);
                    }
                } else {
                    Fabric.getLogger().mo9d(Fabric.TAG, "Could not bind to Google Play Service to capture AdvertisingId");
                }
            } catch (Throwable th) {
                Fabric.getLogger().mo8d(Fabric.TAG, "Could not bind to Google Play Service to capture AdvertisingId", th);
            }
            return null;
        } catch (PackageManager.NameNotFoundException unused) {
            Fabric.getLogger().mo9d(Fabric.TAG, "Unable to find Google Play Services package name");
            return null;
        } catch (Exception e2) {
            Fabric.getLogger().mo8d(Fabric.TAG, "Unable to determine if Google Play Services is available", e2);
            return null;
        }
    }
    ...
    private static final class AdvertisingConnection implements ServiceConnection {
        ...
        public IBinder getBinder() {
            if (this.retrieved) {
                Fabric.getLogger().mo7e(Fabric.TAG, "getBinder already called");
            }
            this.retrieved = true;
            try {
        ...
        public String getId() throws RemoteException {
            ...
            try {
                try {
                    ...
                    Fabric.getLogger().mo9d(Fabric.TAG, "Could not get parcel from Google Play Service to capture AdvertisingId");
                    obtain2.recycle();
                    obtain.recycle();
                    return null;
                }
        ...
        public boolean isLimitAdTrackingEnabled() throws RemoteException {
            ...
            try {
                try {
                    ...
                    Fabric.getLogger().mo9d(Fabric.TAG, "Could not get parcel from Google Play Service to capture Advertising limitAdTracking");
                }
                return z;
            } finally {
                obtain2.recycle();
